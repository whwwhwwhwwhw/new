...
<html lang="en">
    ...
    <br />
			<div class="tabContent">
				<div class="serach">
				<form action="develop/jl_qj.html" method="post">
					<table cellpadding="0" cellspacing="0" class="searchTable" border="0" style="width:60%;">
						<tr>
							<td><h3>所属小组：</h3></td>
							<td><select name="groupid" class="searchStyle">
								<option value="">--请选择--</option>
								<c:forEach var="xz" items="${selectxz}">
									<option value="${xz.id}">${xz.groupname}</option>
								</c:forEach>					
								</select></td>
							<td><h3>姓名：</h3></td>
				...
				<div class="divTable">
					<table cellpadding="0" cellspacing="0" border="0" class="selectTable">
						...
						<c:forEach items="${jlleacelist}" var="f">
						<tr>
							<td>${f.user_id}</td>
							<td>${f.stuname}</td>
							<td>${f.groupname }</td>
							...
							<td> <a href="develop/Qxiangqing.html?user_id=${f.user_id}"
