		...
		<script type="text/javascript">
			$(function() {
				...
				$("#leave").click(function(){
				 	location.href="develop/zy_leaveygadd.html?uid="+${uid};
				});
			});
...
<hr/>
			<div class="tabContent">
				<div class="serach">
				<form action="develop/zy_leavelist.html" method="post">
					<table cellpadding="0" cellspacing="0" class="searchTable" border="0" style="width:60%;">
						<tr>

							<td><h3>请假类型：</h3></td>
							<td><select name="type"><option value="">-- 请选择 --</option>
							<%-- <c:forEach var = "type" items="${selecttype }">
								<option value="${type }">${ }</option>
							</c:forEach> --%>
							
							<option style="width:160px",  value="1">事假</option>
							<option style="width:160px"  value="2">病假</option>
							</select></td>
							<td><input type="submit" value="搜索" class="searchBotton" /></td>
                            <td><input type="button" value="添加" class="searchBotton" id="leave">
				...
				<div class="divTable">
					...
					<c:forEach items="${zyqingjia}" var="f">
						<tr>
							<td>${f.user_id}</td>
							<td>${f.stuname}</td>
