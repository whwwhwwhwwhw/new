...
$(function(){
	...
	window.location.href="Consultway/ToreviseZXS?id="+num;
}
    	var conId = 0;
    	var conId ="";
    	function show(index){
    	$("#resultAll").empty();
			...
			$.post("teacher/consultTeacherQuery",temp,showListInfo,"json");
    			...
    			for ( var i = 0; i < data.length; i++) {
					...
					str+="<tr>";
							...
							str+="<td>"+(i+1)+"</td>"+"<td>"+allotstatus+"</td>"+"<td>"+data[i].name+"</td>"+"<td>"+sex+"</td>"
								...
								+"<td>"+statusName+"</td><td><span style='color: #2828FF;' onclick=ToRevise("+data[i].id+") class='edit'  >完善</span><a href='teacher/consultTeacherVisit?id="+data[i].id+"&stuId=0' class='edit'>"
								+"<span id='edit' style='color: #2828FF;' >回访</span></a>";
						
						/* <a href='javascript:void(0)' onclick=ConDelete("+data[i].id+") class='edit'><span id='edit' style='color: #2828FF;' >删除</span></a> */
	...
	var employee_id=${loginUser.emp_id};
	 var msg = "您真的确定要删除吗？"; 
 if (confirm(msg)==true){ 
 location.href="teacher/ConsulterDelete?id="+Id+"&employee_id="+employee_id;
  return true; 
 }else{ 
  return false; 
 } 
    	...
    	function test(){
    		...
    		$.ajax({
    			url:"teacher/writeResut/"+conId+"/"+conresultInfo+"/"+employee_id,
    			datatype:"json",
		    		...
		    		success:function(res){
		    			 $list=$(res);
		    				...
		    				$.ajax({
								url:"adviceadd",
								type:"GET",
								data:{"from":parent.user,"content":tea+"提交了失效申请","to":id},
								dataType:"json",
				    			success:function(){
				    					var temp="{from:'"+parent.user+"',content:'"+tea+"提交了失效申请',to:'"+id+"'}";
										parent.ws.send(temp);
								        show(indexpage);
				    			}
				    		});
  ...
  /*   	function showInfo1(data){ */
	...
	var consultTeacherName =data[0].consultTeacherName==null?"暂无":data[0].consultTeacherName;
			$("#sss").empty();
			var temp="";
			var num=data.length;
					 for ( var i = 0; i < data.length; i++) {
			 ...
			 $("#sss").append("<tr><td class='tdWidth'>咨询量姓名：</td>"
		                    	...
		                    	+"<input type='button'  value='确定' class='saveBotton' style='margin-top:50px;' /></td></tr>");
		               
                    
			var mbHeight= $(document).height();
		    var mbWidth=$(document).width();
		    var delHeight=$(document).height();
		        
	         $(".mbStyle").css("height",mbHeight); 
	         $(".mbStyle-studyInfo").css("height",mbHeight); 
	         $(".popupStyles").css("left",mbWidth/2-250);
	         $(".popupStyles").css("top",delHeight/3-150);
	         $(".mbStyle").css("display","block");
	         $(".popupStyles").css("display","block");
...
<div class="rightContent" ><!-- style="width:1580px;" -->
    <div class="tabContent">
       ...
       <div class="tabs" id="tabs"  >
        <span class="spanHover" >全部咨询量</span>
       <span class="spanOut" onclick="showone()" >当天咨询量</span>
       <span class="spanOut"  onclick="showthree()" >三日内咨询量</span>
       <span class="spanOut"  onclick="showsave()"  >一周内咨询量</span>
       <span class="spanOut"  onclick="showsaveup()" >一周上咨询量</span>
    	</div>
...
<div class="popupStyle-notsee" style="width: 600px; height: 250px; ">
    ...
    <form action="teacher/invalidConsulter?employee_id=${loginUser.emp_id}" method="post" enctype="multipart/form-data" id="infoform">
