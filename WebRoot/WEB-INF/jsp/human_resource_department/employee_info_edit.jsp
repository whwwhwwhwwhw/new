...
<script type="text/javascript">
	...
	$(function() {
		$("form input.check[type='text']").add(
			...
			$(this).blur(function() {
				...
				$(this).val("").removeClass("redBorder");
			}).keydown(function() {
				$(this).css("color", "gray");
			});
	...
	function getBirthdayAndAge(emp_idcardTag) {
		...
		if (count1 == 1) {
			$("#emp_idcard").removeClass("redBorder");
			...
			if (emp_idcard_val == null || emp_idcard_val == '') {
				$(emp_idcardTag).val("请输入身份证号！").css({
					color : "gray"
				}).addClass("redBorder");
				$("#birthday").val("");
				$("#age").val("");
				...
				if (result == null) {
					$(emp_idcardTag).val("身份证号格式不正确!").css({
						color : "gray"
					}).addClass("redBorder");
				} else {
					checkEmpIdCard(emp_idcard_val, emp_idcardTag);
				}
			}
		}
	}
	...
	/*    验证身份证号是否存在 */
	function checkEmpIdCard(emp_idcard_val, emp_idcardTag) {
		$.ajax({
...
<script type="text/javascript">
	...
	function getProvinceNames() {
		...
		$.post("Employee/GetProvinceList", {
			"ProvinceId" : ProvinceId
		}, showProvinceList1, "text");
		ProvinceId = $("#a_emp_province_02").val();
		$.post("Employee/GetProvinceList", {
			"ProvinceId" : ProvinceId
		}, showProvinceList2, "text");
		ProvinceId = $("#a_emp_province_03").val();
		$.post("Employee/GetProvinceList", {
			"ProvinceId" : ProvinceId
		}, showProvinceList3, "text");
	}
	...
	function showCityList1(data) {
		...
		/* 		 alert(data);   */
		/* 		$("#emp_county_01").html("<option value='0'>请选择</option>"); */
	...
	function getDepartmentNames() {
		...
		$.post("Employee/GetDepartmentList", {
			"DepartmentName" : DepartmentName
		}, showDepartmentList, "text");
	}
	function showDepartmentList(data) {
		/* 	$("#emp_position").html("<option value=''>请选择</option>"); */
		/* alert(data);  */
	...
	function cloneTd(cloneButton) {
		...
		var currentTr = "<tr height='100px'><td>技术职称:"
			+ num
			+ "</td><td> <input type='text'  placeholder='请上传格式为：jpg,png,jpeg,pdf的文件'    style='width: 300px;'    class='textStyle' id='imgUrl' style='width:400px;'/><input type='file'  name='a_emp_technical_title_path' onchange='showFilePath(this)' /><input type='button'  value='上传'  onclick='ajaxFileUpload1(this)'  /><span  id='message' ></span><img alt='图片丢失'   width='100px' height='100px' src='' style='display: none;vertical-align: middle;'><input type='button' value='继续添加' onclick='cloneTd(this)'></td></td></tr>";
	...
	/* 收起证件及经历表单 */
	/* var num = 1;
	function showTable(showButton) {
		if (num % 2 == 1) {
			$(showButton).next().slideDown();
			num++;
			$(showButton).val("收起");

		} else {
			$(showButton).next().slideUp();
			num++;
			$(showButton).val("展开");
		}
	}
	var num1 = 1;
	function showTable1(showButton) {
		if (num1 % 2 == 1) {
			$(showButton).next().slideDown();
			num1++;
			$(showButton).val("收起");
	...
	function uploadPersonPhoto(uploadFile) {
		...
		setInt = setInterval(function() {
			...
			if (zhaopianmiao == 0) {
				...
				$(uploadFile).next().html("");
				clearInterval(setInt);
			}
		}, 1000);
		$.ajax({
			url : "Employee/UploadPersonPhoto/" + emp_id + "", //请求的url地址
			dataType : "text", //返回格式为json
			async : true, //请求是否异步，默认为异步，这也是ajax重要特性
			type : "POST", //请求方式
			data : {
	...
	function saveInfo(formInfo) {
		/* alert(emp_idcard); */
		var a_emp_idcard = $("#emp_idcard").val();
		var emp_id = $("#emp_id").val();
		...
		if (formIndex == 0) {
			...
			if (emp_name_val.match(emp_name_val_reg) == null) {
				...
				$("#emp_now_detail").val("请输入现住址详细信息!").css("color", "gray").addClass("redBorder");
				return;
			} else if (emp_origin_detail_val == null || emp_origin_detail_val == "") {
				$("#emp_origin_detail").val("请输入现住址详细信息!").css("color", "gray").addClass("redBorder");
				return;
			} else {
				$.ajax({
	...
	function ajaxFileUpload(upload) {
		...
		var img = $(upload).parent().next();
		$(upload).next().html("文件上传中！").css("color", "red");
		/*    alert(file[0].files[0]); */
		formData.append("file", file[0].files[0]);
		formData.append("fileName", fileName);
		formData.append("emp_idcard", emp_idcard);
		$.ajax({
			url : "Employee/AddEmployeeFile/AddCertificateInfo/0/" + emp_id + "",
			type : "POST",
			cache : false,
			data : formData,
			dataType : "json",
	...
	function ajaxFileUpload1(upload) {
		var emp_id = $("#emp_id").val();
		var emp_idcard = $("#emp_idcard").val();
		var formData = new FormData();
		var fileName = $(upload).prev().attr("name");
...
<script type="text/javascript">
	function showUploadFileMenu(showButton) {
		$(showButton).prev().prev().css("display", "inline-block");
		$(showButton).hide();
	}
	function showUploadFileMenu1(showButton) {
		$(showButton).prev().css("display", "inline-block");
		$(showButton).hide();
	}
</script>
<style>
				...
				<form action="Employee/AddEmployee/AddBasicInfo" method="post">
					...
					<table cellpadding="0" cellspacing="0" border="0" class="addTable"
						...
						<tr>
							...
							<td style="width: 190px;"><input type="search"
								...
								style="width:160px;" value="${oaRlzybEmployee.emp_name }"
								placeholder="请输入中文姓名" /></td>
							<td style="width: 160px;"><span>*</span>身份证号：</td>
							<td colspan="2"><input type="search" name="emp_idcard"
								placeholder="请输入公民身份证号" id="emp_idcard" class="textStyle check"
								style="width:160px;display: inline-block;"
								onblur="getBirthdayAndAge(this)"
								value="${oaRlzybEmployee.emp_idcard}" /><span
								style="display: inline-block;" id="checkResult"></span></td>

							<td rowspan="4" colspan="2" align="center"><img id="snapimg"
								onerror="javascript:this.src='statics/common/images/loseImageIcon.png' "
								alt="图片丢失" src="${oaRlzybEmployee.emp_photo_path }" width="209"
								height="209" style="border: black solid 1px;"></td>
							<td style="width: 129px;"></td>
						<tr>
							<td><span>*</span>性别：</td>
							<td><c:if test="${oaRlzybEmployee.emp_sex eq '男' }">
									<input type="radio" value="男" name="emp_sex" class="radioStyle"
										checked="checked">男&nbsp;
								<input type="radio" value="女" name="emp_sex" class="radioStyle">女&nbsp;	
							</c:if> <c:if test="${oaRlzybEmployee.emp_sex eq '女' }">
									<input type="radio" value="女" name="emp_sex" class="radioStyle"
										checked="checked">女&nbsp;
							<input type="radio" value="男" name="emp_sex" class="radioStyle">男&nbsp;	
							</c:if> <input readonly="readonly" type="text" name="emp_name_simple"
						...
						<tr>
							...
							<td><select name="emp_educational_background"
								style="width:160px;" class="selectStyle ">
									<c:if
										test="${oaRlzybEmployee.emp_educational_background eq '本科' }">
									...
									<c:if
										test="${oaRlzybEmployee.emp_educational_background eq '专科' }">
									...
									<c:if
										test="${oaRlzybEmployee.emp_educational_background eq '高中' }">
									...
									<c:if
										test="${oaRlzybEmployee.emp_educational_background eq '硕士' }">
						...
						<tr>
							...
							<td><input type="tel" name="emp_phone"
								onblur="checkPhoneNumber()" placeholder="请输入手机号码"
								value="${oaRlzybEmployee.emp_phone }" class="textStyle check"
								id="emp_phone" style="width:160px;"></td>
							<td><span>*</span>邮箱：</td>
							<td><input type="email" placeholder="请输入您常用的邮箱地址"
								onblur="checkEmail()" name="emp_Email" class="textStyle check"
								value="${oaRlzybEmployee.emp_Email }" id="emp_Email"
						...
						<tr>
							<td><span>*</span>民族：</td>
							<td><input type="hidden" id="a_nation_name"
								value="${oaRlzybEmployee.nation_name }" /> <select
								name="nation_name" id="nation_name" class="selectStyle "
								style="width:160px;">
									<option value="0">请选择</option>
							</select></td>
							<td>微信账号：</td>
							<td><input type="text" name="emp_hobby" class="textStyle"
								value="${oaRlzybEmployee.emp_hobby }" style="width: 160px;" /></td>


							<td style="text-align: center"></td>
							<td colspan="2" align="center"><input type="button"
								height:="" px="" id="showTakePhotoPage"
								onclick="confirmEmpIdcard()" value="拍照/上传个人照片"></td>
							<td></td>
							<td></td>
						</tr>
						<tr>
							<td><span>*</span>籍贯：</td>
							<td><input type="hidden" id="a_emp_province_01"
								value="${oaRlzybEmployee.emp_province }" /> <select
								name="emp_province" onchange="getCityNames(this)"
								id="emp_province_01" mold="province1" class="selectStyle "
								style="width:160px;">
									<option value="0">请选择</option>
							</select></td>
							<td><input type="hidden" id="a_emp_city_01"
								value="${oaRlzybEmployee.emp_city }" /> <select name="emp_city"
								id="emp_city_01" onchange="getAreaNames(this)"
								class="selectStyle " style="width: 160px;" mold="city1">
									<option value="0">请选择</option>
							</select></td>
							<td><input type="hidden" id="a_emp_county_01"
								value="${oaRlzybEmployee.emp_county }" /> <select
								name="emp_county" id="emp_county_01" class="selectStyle "
								style="width:160px;">
									<option value="0">请选择</option>
							</select></td>
							<td>籍贯补充内容：</td>
							<td colspan="2"><input type="text" name="emp_origin_detail"
								placeholder="请输入现住址详细信息" id="emp_origin_detail"
								class="textStyle check" style="width: 300px;"
								value="${oaRlzybEmployee.emp_origin_detail }"></td>
							<td></td>
							<td></td>
						...
						<tr>
							<td><span>*</span>现住址：</td>
							<td><input type="hidden" id="a_emp_province_02"
								value="${oaRlzybEmployee.emp_now_province }" /> <select
								name="emp_now_province" onchange="getCityNames(this)"
								id="emp_province_02" class="selectStyle " style="width:160px;"
								mold="province2">
									<option value="0">请选择</option>
							</select> <!-- <textarea name="emp_address"
									class="areaStyle" style="width:500px;"></textarea> --></td>
							<td><input type="hidden" id="a_emp_city_02"
								value="${oaRlzybEmployee.emp_now_city }" /> <select
								name="emp_now_city" id="emp_city_02"
								onchange="getAreaNames(this)" class="selectStyle "
								style="width: 160px;" mold="city2">
									<option value="0">请选择</option>
							</select></td>
							<td><input type="hidden" id="a_emp_county_02"
								value="${oaRlzybEmployee.emp_now_county }" /> <select
								id="emp_county_02" name="emp_now_county" class="selectStyle "
								style="width:160px;">
									<option value="0">请选择</option>
							</select></td>
							<td>现住址补充内容：</td>
							<td colspan="2"><input type="text" name="emp_now_detail"
								placeholder="请输入现住址详细信息" id="emp_now_detail"
								class="textStyle check" style="width: 300px;"
								value="${oaRlzybEmployee.emp_now_detail }"></td>
							<td></td>
							<td></td>
						</tr>
					</table>
					<div class="btposition1">
						<input type="button" class="buttonStyle" id="saveBasicInfo"
							onclick="saveInfo(this)" formIndex="0" value="保存" /><span></span>
					</div>
				</form>
			</div>
			<div class="divTable" style="display:none;" id="tab2">
				<table cellpadding="0" cellspacing="0" border="0" class="addTable"
					...
					<tr height="100px">
						...
						<td style="width: 1000px; vertical-align:middle;"><span
							style="display: none;color: gray;"><input type="text"
								class="textStyle" id="imgUrl" style="width: 300px"
								placeholder="请上传格式为：jpg,png,jpeg,pdf的文件" /> <input
								name="a_emp_idcard_front" type="file"
								onchange="showFilePath(this)" /> <input type="button"
								value="上传" onclick="ajaxFileUpload(this)" /><span
								id="message"></span></span> <c:choose>
								<c:when test="${oaRlzybEmployee.emp_idcard_front eq '空'}">
									<img alt="图片丢失" src="" width="100px" height="100px"
										style="vertical-align: middle;display: none;">
								</c:when>
								<c:otherwise>
									<img alt="图片丢失" width="100px" height="100px"
										onerror="javascript:this.src='statics/common/images/loseImageIcon.png' "
										src="${oaRlzybEmployee.emp_idcard_front }"
										style="vertical-align: middle;" />
								</c:otherwise>
							</c:choose> <input type="button" value="重新上传"
							onclick="showUploadFileMenu(this)" /></td>
					</tr>

					<tr height="100px">
						<td><span>*</span>身份证反面:</td>
						<td><span style="display: none;color: gray;"><input
								type="text" class="textStyle" id="imgUrl"
								placeholder="请上传格式为：jpg,png,jpeg,pdf的文件" style="width: 300px" />
								<input type="file" name="a_emp_idcard_reverse"
								onchange="showFilePath(this)" /> <input type="button"
								value="上传" onclick="ajaxFileUpload(this)" /><span id="message"></span></span>
							<c:choose>
								...
								<c:otherwise>
									<img alt="图片丢失" src="${oaRlzybEmployee.emp_idcard_reverse }"
										width="100px"
										onerror="javascript:this.src='statics/common/images/loseImageIcon.png' "
					...
					<tr height="100px">
						...
						<td><span style="display: none;color: gray;"><input
							...
							<c:choose>
								...
								<c:otherwise>
									<img alt="图片丢失"
										src="${oaRlzybEmployee.emp_Degree_certificate }" width="100px"
										height="100px"
										onerror="javascript:this.src='statics/common/images/loseImageIcon.png' "
										style="vertical-align: middle;">
								</c:otherwise>
					...
					<tr height="100px">
						<td>学历证:</td>
						<td><span style="display: none;color: gray;"><input
								type="text" name="a_emp_Education_certificate" class="textStyle"
								id="imgUrl" style="width: 300px"
								...
								<c:otherwise>
									<img alt="图片丢失"
										src="${oaRlzybEmployee.emp_Education_certificate }"
										onerror="javascript:this.src='statics/common/images/loseImageIcon.png' "
					...
					<tr height="100px">
						<td>技术职称列表：</td>
						<td><c:forEach items="${oaRlzybTechnicalTitleList}"
								var="oaRlzybTechnicalTitle">
								<img alt="图片丢失" width="100px" height="100px"
									src="${oaRlzybTechnicalTitle.technical_title_path}"
									style="vertical-align: middle;"
									onerror="javascript:this.src='statics/common/images/loseImageIcon.png' ">
							</c:forEach></td>
					</tr>
					<tr height="100px">
						...
						<td><span style="display: none;color: gray;"><input
								...
								onchange="showFilePath(this)" /> <input type="button"
								value="上传" onclick="ajaxFileUpload1(this)" /><span id="message"></span><img
								alt="图片丢失" width="100px" height="100px" src=""
								style="display: none;vertical-align: middle;"> <input
								type="button" value="继续添加" onclick="cloneTd(this)"></span> <input
							type="button" value="重新上传" onclick="showUploadFileMenu1(this)" />
				...
				<form action="Employee/AddEmployee/AddPositionInfo" method="post">
					...
					<table cellpadding="0" cellspacing="0" border="0" class="addTable">
						<tr>
							...
							<td style="width: 190px;"><input type="hidden"
								id="a_emp_department" value="${oaRlzybEmployee.emp_department }" />
								<select class="selectStyle " onchange="getPositionNames(this)"
								name="emp_department" id="emp_department" style="width:160px;">
									<option value="0">请选择</option>
							</select></td>
							<td style="width: 120px;"><span>*</span>职位：</td>
							<td style="width: 190px;"><input type="hidden"
								id="a_emp_position" value="${oaRlzybEmployee.emp_position }" />
								<select class="selectStyle " name="emp_position"
								id="emp_position" style="width:160px;">
									<option value="0">请选择</option>
							</select></td>
							<td style="width: 120px;"><span>*</span>用工方式：</td>
							<td style="width: 190px;"><c:if
									test="${oaRlzybEmployee.emp_Employment_nature eq '全职' }">
									<input type="radio" value="全职" name="emp_Employment_nature"
										class="radioStyle" checked="checked" />全职&nbsp;&nbsp;&nbsp;&nbsp; <input
										type="radio" value="兼职" name="emp_Employment_nature"
										class="radioStyle" />兼职</td>
							</c:if>
							<c:if test="${oaRlzybEmployee.emp_Employment_nature eq '兼职' }">
								<input type="radio" value="全职" name="emp_Employment_nature"
									class="radioStyle" />全职&nbsp;&nbsp; <input
									type="radio" value="兼职" checked="checked"
									name="emp_Employment_nature" class="radioStyle" />兼职</td>
							</c:if>
						</tr>
						<tr>
							<td><span>*</span>入职日期：</td>
							<td><input type="date" name="a_emp_Entry_date"
								class="dateStyle " style="width:160px;"
								value="${oaRlzybEmployee.emp_Entry_date_str}" /></td>
							...
							<td><input type="date" name="a_emp_tryout_end_date"
								class="dateStyle " style="width:160px;"
								value="${oaRlzybEmployee.emp_tryout_end_date_str}" /></td>

							<td><span>*</span>目前状态：</td>
							<td><c:if test="${oaRlzybEmployee.emp_now_status eq '在职' }">
									<input type="radio" value="在职" class="radioStyle"
										name="emp_now_status" checked="checked" />在职&nbsp;&nbsp;
								<input type="radio" value="离职" name="emp_now_status"
										class="radioStyle" />离职&nbsp;&nbsp;
								<input type="radio" value="停薪留职" name="emp_now_status"
										class="radioStyle" />停薪留职
								</td>
							</c:if>
							<c:if test="${oaRlzybEmployee.emp_now_status eq '离职' }">
								<input type="radio" value="在职" class="radioStyle"
									name="emp_now_status" />在职&nbsp;&nbsp;
								<input type="radio" value="离职" name="emp_now_status"
									class="radioStyle" checked="checked" />离职</td>
								&nbsp;&nbsp;
								<input type="radio" value="停薪留职" name="emp_now_status"
									class="radioStyle" />停薪留职
							</c:if>
							<c:if test="${oaRlzybEmployee.emp_now_status eq '停薪留职' }">
								<input type="radio" value="在职" class="radioStyle"
									name="emp_now_status" />在职&nbsp;&nbsp;
								<input type="radio" value="离职" name="emp_now_status"
									class="radioStyle" />离职</td>
								&nbsp;&nbsp;
								<input type="radio" checked="checked" value="停薪留职"
									name="emp_now_status" class="radioStyle" />停薪留职
							</c:if>
						</tr>
						<tr>
							<td>技术职称：</td>
							<td><input type="text" name="emp_technical_title_name"
								class="textStyle" style="width:160px;"
								value="${oaRlzybEmployee.emp_technical_title_name}" /></td>

							<td><span>*</span>员工等级：</td>
							<td><input type="hidden" id="a_emp_grade_id"
								value="${oaRlzybEmployee.emp_grade_id }" /> <select
								name="emp_grade_id" id="emp_grade_id" class="selectStyle "
								style="width:160px;">
									<option value="0">请选择</option>
							</select></td>
						...
						<tr>
							...
							<td><input type="number" name="emp_salary_cardnumber"
								class="textStyle" style="width:160px;"
								value="${oaRlzybEmployee.emp_salary_cardnumber }" /></td>
							<td>医保卡号：</td>
							<td><input type="number"
								name="emp_medical_insurance_cardnumber" class="textStyle"
								style="width:160px;"
								value="${oaRlzybEmployee.emp_medical_insurance_cardnumber }" /></td>
							<td>养老保险卡号：</td>
							<td><input type="number"
								name="emp_endowment_insurance_cardnumber" class="textStyle"
								style="width:160px;"
								value="${oaRlzybEmployee.emp_endowment_insurance_cardnumber }" /></td>
						</tr>
					</table>
					<div class="btposition1">
						<input type="button" class="buttonStyle" onclick="saveInfo(this)"
					...
					<table cellpadding="0" cellspacing="0" border="0" class="addTable">
						<tr>
							...
							<td style="width: 190px;"><input type="text"
								...
								value="${oaRlzybEmployee.emp_contact_name }"
								style="width:160px;" placeholder="请输入中文姓名" /></td>

							<td style="width: 190px;"><span>*</span>手机号码：</td>
							<td style="width: 190px;"><input type="tel"
								placeholder="请输入手机号码" id="emp_contact_phone"
								name="emp_contact_phone" class="textStyle check"
								value="${oaRlzybEmployee.emp_contact_phone }"
								style="width:160px;" /></td>
							...
							<td style="width: 270px;"><input type="search"
								...
								style="width:160px;" value="${oaRlzybEmployee.emp_contact_relationship}"/></td>
						</tr>
						<tr>
							<td><span>*</span>现住址：</td>

							<td><input type="hidden" id="a_emp_province_03"
								value="${oaRlzybEmployee.emp_contact_address_province }" /> <select
								name="emp_contact_address_province"
								onchange="getCityNames(this)" id="emp_province_03"
								class="selectStyle " style="width:160px;" mold="province3">

							</select></td>
							<td><input type="hidden" id="a_emp_city_03"
								value="${oaRlzybEmployee.emp_contact_address_city }" /> <select
								name="emp_contact_address_city" id="emp_city_03"
								onchange="getAreaNames(this)" class="selectStyle "
								style="width: 160px;" mold="city3">

							</select></td>
							<td><input type="hidden" id="a_emp_county_03"
								value="${oaRlzybEmployee.emp_contact_address_county }" /> <select
								id="emp_county_03" name="emp_contact_address_county"
								class="selectStyle " style="width:160px;">

							</select></td>
							<td>现住址补充内容：</td>
							<td><input type="text" name="emp_contact_address_detail"
								placeholder="请输入现住址详细信息" id="emp_contact_address_detail"
								class="textStyle check" style="width:300px;"
								value="${oaRlzybEmployee.emp_contact_address_detail }"></td>
						</tr>
					</table>
					<div class="btposition1">
						<input type="button" class="buttonStyle" onclick="saveInfo(this)"
